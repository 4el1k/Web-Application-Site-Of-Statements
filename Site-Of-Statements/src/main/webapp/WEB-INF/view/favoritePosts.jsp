...
<head>
    ...
    <style><%@include file="/style/stylesFavoritePosts.css" %> </style>
</head>
<body>
<div>
    <div class="menu">
    ...
    <div class="scroll">
        <div class="feed">
            <%List<Post> posts = (List<Post>) request.getAttribute("favoritePosts"); %>
            <%Account account;%>
            <% for (Post post : posts) { %>
            <div class="frame">
                <div>
                    <p class="title">Title: <%=post.getTitle()%>
                    </p>
                    <p class="price">Price: <%=post.getPrice()%> Rubles</p>
                    <p class="description">Description: <%=post.getDescription()%>
                    </p>
                    <div class="account">
                        <%account = post.getAccount();%>
                        <p class="account-name">Account name: <%=account.getName()%>
                        </p>
                        <p class="account-phone">Account phone: <%=account.getPhoneNumber()%>
                        </p>
                    </div>
                    <div>
